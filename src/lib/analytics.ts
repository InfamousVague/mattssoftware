/// Plausible wrapper for the mattssoftware.com marketing site.
///
...
///   2. Custom events. CTA clicks, download attempts, language
///      switches, etc. The wrapper keeps the dispatch site tight
///      (`trackEvent("download.click", { app: "blip" })`) and
///      provides a single chokepoint for the type signature, the
///      no-op fallback if the script is blocked, and any future
...
/// endpoint both live on `stats.mattssoftware.com` (same root
/// domain as this site) so corporate firewalls treat them as
/// first-party and the request doesn't trip cross-origin filters.
///
/// Mirrors the libre.academy/src/lib/analytics.ts contract so
/// both sites share the same call sites — `trackPageview()`,
...
/// `props` bag carries the structured payload (`{ app: "blip" }`
